...
ANA_TEST_final = function(my_dirs,nPERM = 1e5){
	...
	euc_fn = file.path(my_dirs$spms_dir,"euc.rds")
...
#'	processing, and analyzing real data
#' @export
new_ANA_AGG = function(git_dir,work_dir){
	
	my_dirs0 = setdirs(git_dir = git_dir,
		work_dir = work_dir,dataset = "COAD")
	res_fn = file.path(my_dirs0$fin_dir,"pan_kern.rds")
	...
	if( !file.exists(res_fn) ){
		...
		for(dataset in datasets){
			...
			my_dirs = setdirs(git_dir = git_dir,
				work_dir = work_dir,dataset = dataset,
				verbose = FALSE)
			
			tum_fns = list.files(my_dirs$reg_dir)
			...
			for(fn in tum_fns){
				# fn = tum_fns[1]
				tmp_df = readRDS(file.path(my_dirs$reg_dir,fn))
	...
	if( TRUE ){
		...
		dim(MAT); MAT[1:4,]
		png(file.path(my_dirs0$fin_dir,"null_models.png"),units = "px",
	...
	if( TRUE ){
		...
		png_fn = file.path(my_dirs0$fin_dir,"log10_TMB.png")
	...
	if( TRUE ){
		...
		gg = ggplot(data = dat,mapping = aes(x = YY_EUC,y = YY_OT,group = dataset2)) +
			...
			my_theme
		png_fn = file.path(my_dirs0$fin_dir,"overall_omnibus_scatter.png")
	...
	if( TRUE ){
		...
		# gg
		png_fn = file.path(my_dirs0$fin_dir,"top_tumor_regs.png")
	...
	if( TRUE ){ # Check top significant tumor types
		...
		for(dataset in datasets){
			...
			png_fn = file.path(my_dirs0$fin_dir,sprintf("indepth_%s.png",dataset))
